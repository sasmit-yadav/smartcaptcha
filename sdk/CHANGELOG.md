...
### Security
- Stack mild environment-coherence signals (viewport, touch, plugins,
  deviceMemory, Gecko buildID under Chrome UA) so several soft tells can
  cross the block threshold together — closer to industry multi-signal
  fingerprinting without a challenge step.

## [1.1.8] - 2026-07-24

### Security
- Environment coherence probes (UA↔engine↔WebGL↔platform) for patched
  browsers that pass classic webdriver checks (Camoufox-class). Strong
  mismatches raise `automation_score` / `webdriverFlag`; mild mismatches
  stay soft (< block threshold).

## [1.1.7] - 2026-07-24

### Fixed
- Republish of 1.1.6 contents after npm reserved 1.1.6 without a public
  dist-tag (registry conflict). Same signing-error retry + CDP inconclusive
  behavior as 1.1.6.
